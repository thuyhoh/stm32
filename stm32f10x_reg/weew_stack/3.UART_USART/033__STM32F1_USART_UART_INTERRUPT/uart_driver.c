...
void UART_ISR(unsigned short uart, unsigned short bridge, unsigned short* signal, unsigned short* counter, char msg[]){
	// khi ko la cau noi giua hai thiet bi
	if(bridge == 0){ 
		msg[*counter] = UART_Rx(uart); // nhan thong tin tu USARTx
		if(msg[*counter] == '\n'){
			*counter = 0;
			*signal = 1;
		}else{
	...
	// khi la cau noi giua hai thiet bi
		...
		UART_Tx(bridge,UART_Rx(uart)); // truyen du lieu nhan duoc tu chan UASRT den USART(bridge)
	}
