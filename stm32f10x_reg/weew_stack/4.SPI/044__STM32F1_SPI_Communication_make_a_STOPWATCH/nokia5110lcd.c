...
void n5110_add_print(unsigned short Spi_Channel,short Ypos, short Xpos, char str[]){
...
void update_buffer(ImgType img, unsigned short img_nums, unsigned char scream_buffer[][84]){
	...
	for(y_dir = img.Y_pos; y_dir <= endy; y_dir++){
		for(x_dir = img.X_pos; x_dir <= endx; x_dir++){
			cnt = (y_dir - img.Y_pos)*img.W + (x_dir - img.X_pos); // phan tu thu bao nhieu cua mang
			scream_buffer[y_dir][x_dir] = img.image[img_nums][cnt];// img thu may trong struct;
