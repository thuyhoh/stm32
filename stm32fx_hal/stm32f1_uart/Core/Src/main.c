...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
	HAL_UART_Receive_IT(&huart1,&pdata,1); // khi nhan duoc 1 ky tu -> ham callback
  /* USER CODE END 2 */
...
void set_led(uint8_t led,uint8_t status)
{
	HAL_GPIO_WritePin(GPIOA, 1<< led, status);
}
